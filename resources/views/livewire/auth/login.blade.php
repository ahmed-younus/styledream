<div class="min-h-screen flex items-center justify-center pt-20 pb-12 px-4">
    <div class="w-full max-w-md">
        ...
        <div class="text-center mb-8">
            <h1 class="text-3xl font-bold text-foreground mb-2">{{ __('auth.welcome_back') }}</h1>
            <p class="text-muted-foreground">{{ __('auth.sign_in_to_continue') }}</p>
        </div>
        ...
        <div class="relative mb-6">
            ...
            <div class="relative flex justify-center text-sm">
                <span class="px-4 bg-background text-muted-foreground">{{ __('auth.or_continue_email') }}</span>
            </div>
        </div>
        ...
        <form wire:submit="login" class="space-y-4">
            ...
            <div class="flex items-center justify-between">
                <label class="flex items-center gap-2 cursor-pointer">
                    <input type="checkbox" wire:model="remember" class="w-4 h-4 rounded border-border text-primary focus:ring-primary">
                    <span class="text-sm text-muted-foreground">{{ __('auth.remember_me') }}</span>
                </label>
                <a href="#" class="text-sm text-primary hover:underline">{{ __('auth.forgot_password') }}</a>
            </div>
        ...
        <p class="mt-6 text-center text-sm text-muted-foreground">
            {{ __('auth.no_account') }}
            <a href="{{ route('register') }}" class="text-primary font-medium hover:underline">{{ __('auth.sign_up') }}</a>
        </p>
    </div>
</div>
