        ...
        @if($items->count() > 0)
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
                @foreach($items as $item)
                    <div x-data="{ fav: {{ $item->is_favorite ? 'true' : 'false' }}, removing: false }"
                         x-show="!removing"
                         x-transition:leave="transition ease-in duration-200"
                         x-transition:leave-start="opacity-100 scale-100"
                         x-transition:leave-end="opacity-0 scale-95"
                         class="bg-secondary rounded-xl overflow-hidden group relative">
                        {{-- Image --}}
                        <div class="aspect-square overflow-hidden relative">
                            <img src="{{ $item->image_url }}" alt="{{ $item->name }}" class="w-full h-full object-cover group-hover:scale-105 transition-transform" loading="lazy">
                            ...
                            <div class="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity hidden lg:flex items-center justify-center gap-2">
                                <button @click="fav = !fav; $wire.toggleFavorite({{ $item->id }})" class="p-2 bg-white rounded-full hover:bg-gray-100 transition-colors">
                                    <svg x-show="fav" class="w-5 h-5 text-red-500" fill="currentColor" viewBox="0 0 24 24">
                                        <path d="M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 2 5.42 4.42 3 7.5 3c1.74 0 3.41.81 4.5 2.09C13.09 3.81 14.76 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.78-3.4 6.86-8.55 11.54L12 21.35z"/>
                                    </svg>
                                    <svg x-show="!fav" class="w-5 h-5 text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z"/>
                                    </svg>
                                </button>
                                <button @click="if(confirm('{{ __('wardrobe.delete_confirm') }}')) { removing = true; $wire.deleteItem({{ $item->id }}) }" class="p-2 bg-white rounded-full hover:bg-gray-100 transition-colors">
                            ...
                            <div class="absolute top-1.5 right-1.5 flex gap-1.5 lg:hidden">
                                <button @click="fav = !fav; $wire.toggleFavorite({{ $item->id }})" class="p-2.5 bg-white/90 backdrop-blur-sm rounded-full shadow-sm active:scale-95 transition-transform">
                                    <svg x-show="fav" x-cloak class="w-4 h-4 text-red-500" fill="currentColor" viewBox="0 0 24 24">
                                        <path d="M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 2 5.42 4.42 3 7.5 3c1.74 0 3.41.81 4.5 2.09C13.09 3.81 14.76 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.78-3.4 6.86-8.55 11.54L12 21.35z"/>
                                    </svg>
                                    <svg x-show="!fav" class="w-4 h-4 text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z"/>
                                    </svg>
                                </button>
                                <button @click="if(confirm('{{ __('wardrobe.delete_confirm') }}')) { removing = true; $wire.deleteItem({{ $item->id }}) }" class="p-2.5 bg-white/90 backdrop-blur-sm rounded-full shadow-sm active:scale-95 transition-transform">
                            ...
                            <div x-show="fav" x-cloak class="absolute top-2 right-2 hidden lg:block pointer-events-none">
